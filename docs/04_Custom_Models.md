# 04. 自定义模型导入 (Custom Models)

## 课程概述

本课程介绍如何在 Babylon.js 中导入自定义 3D 模型。这是开发 3D 应用程序的核心技能之一，因为大多数游戏和应用都需要使用外部制作的 3D 资产。

### 学习目标

- 掌握 `SceneLoader` 的使用方法
- 了解支持的模型格式及其特点
- 理解模型加载的异步处理
- 学会在 Blender 中正确导出模型
...
`SceneLoader` 是 Babylon.js 用于加载外部 3D 模型文件的核心 API。
...
SceneLoader.ImportMesh(
  ...
  "./models/",     // rootUrl: 模型目录
  "barrel.glb",    // sceneFilename: 模型文件名
  this.scene,      // scene: 目标场景
  (meshes) => {    // onSuccess: 成功回调
    console.log("加载成功", meshes);
  }
);
```

**方式二：异步方式 (ImportMeshAsync) - 推荐**
...
| `rootUrl` | string | 模型文件所在目录路径（必须以 `/` 结尾） |
...
| 格式 | 扩展名 | 特点 |
|------|--------|------|
| **GLB** | `.glb` | 二进制格式，文件小，推荐用于生产环境 |
| **GLTF** | `.gltf` | JSON 格式，可读性好，适合调试 |
| **OBJ** | `.obj` | 传统格式，兼容性好，但不支持动画 |
