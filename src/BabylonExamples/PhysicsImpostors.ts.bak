...
export class PhysicsImpostors {
  ...
  CreateImpostors(): void {
    ...
    // 1. 创建盒子（动态物理对象）
    // ========================================
    // MeshBuilder.CreateBox 用于创建立方体网格
    const box = MeshBuilder.CreateBox("box", { size: 2 });

    // 设置盒子的初始位置
    // Y=10 让盒子从高处落下，便于观察物理效果
    box.position = new Vector3(0, 10, 1);

    // 设置盒子的初始旋转
    // X 轴旋转 45 度（PI/4），让下落过程更有趣
    box.rotation = new Vector3(Math.PI / 4, 0, 0);

    // 为盒子创建物理模拟器
    //
    // PhysicsImpostor 构造函数参数：
    //   - object: 要应用物理的网格对象
    //   - impostorType: 模拟器类型（决定碰撞形状）
    //   - options: 物理属性配置
    //
    ...
    // 2. 创建地面（静态物理对象）
    // ========================================
    // MeshBuilder.CreateGround 创建一个平面作为地面
    ...
    // 关键配置：
    //   - mass: 0  质量为 0，表示静态对象
    //           静态对象不会移动，但其他物体可以与它碰撞
    //   - restitution: 0.5  中等弹性
    //
    // 注意：地面也使用 BoxImpostor 而不是 PlaneImpostor
    // 因为 BoxImpostor 更稳定，适合作为平坦表面
    ...
    // 3. 创建球体（动态物理对象）
    // ========================================
    // MeshBuilder.CreateSphere 创建球体网格
    ...
    //
    // PhysicsImpostor 类型：
    //   - BoxImpostor: 盒形碰撞器，适合方形物体
    //   - SphereImpostor: 球形碰撞器，适合球体
    //   - CylinderImpostor: 圆柱形碰撞器
    //   - PlaneImpostor: 平面碰撞器（单面）
    //   - ConvexHullImpostor: 凸包碰撞器，适合复杂形状
    //   - MeshImpostor: 网格碰撞器，最精确但性能消耗大
    //
    // 性能建议：
    //   - 始终选择最简单的碰撞器类型
    //   - 复杂模型使用 ConvexHull 而非 Mesh
    //   - 静态场景元素使用 mass: 0
    //
    // 弹性系数（restitution）说明：
    //   - 0: 无弹性，碰撞后不反弹（如泥巴）
    //   - 0.5: 中等弹性（如篮球）
    //   - 1: 完全弹性，碰撞后反弹到原高度（理想状态）
    //
    // 注意：两个物体的弹性系数会相互影响
    // 例如：盒子 0.75 + 地面 0.5 的组合效果会介于两者之间
  }
}