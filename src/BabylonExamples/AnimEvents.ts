import {
  ...
  CubeTexture,
  SceneLoader,
  AnimationEvent,  // 动画事件类，用于在动画特定帧触发回调
  AnimationGroup,  // 动画组，包含一组相关动画
} from "@babylonjs/core";
...
export class AnimEvents {
  ...
  // animationGroups[0] = death（死亡动画）
  // animationGroups[1] = idle（待机动画）
  ...
  async CreateCharacter(): Promise<void> {
    ...
    // - action: 事件触发时执行的回调函数
    // - onlyOnce: 是否只触发一次（false = 每次动画播放都会触发）
  ...
  async CreateZombie(): Promise<void> {
    ...
    // ============================================
    // 停止死亡动画，播放待机动画
    this.zombieAnims[0].stop();  // death animation
    this.zombieAnims[1].play(true);  // idle animation, 循环播放
